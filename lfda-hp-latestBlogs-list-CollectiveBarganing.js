...
document.write('<div id="hpLatestIssueBlogs"></div>');

jQuery(document).ready(function(){
	// Add a "raw zone" to the end of the BODY tag to store the raw data coming from the feed.
	jQuery('body').append('<div id="hpLatestIssueBlogsRaw" style="display: none;"></div>')
	// Now, get the feed and do some processing once i's done.
	jQuery("#hpLatestIssueBlogsRaw").load("/rAjaxGW/rget.aspx?action=ning-blog-list-CollectiveBarganing", function(response, status, xhr) {
		...
		firstFewItems=jQuery("#hpLatestIssueBlogsRaw div.xg_module_body").filter(":lt(3)");
		...
		jQuery(firstFewItems).appendTo("#hpLatestIssueBlogs");

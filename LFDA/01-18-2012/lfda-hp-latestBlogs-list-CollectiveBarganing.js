...
document.write('<div id="hpLatestBlogsCollectiveBarganing"></div>');

jQuery(document).ready(function(){
	// Add a "raw zone" to the end of the BODY tag to store the raw data coming from the feed.
	jQuery('body').append('<div id="hpLatestBlogsCollectiveBarganingRaw" style="display: none;"></div>')
	// Now, get the feed and do some processing once i's done.
	jQuery("#hpLatestBlogsCollectiveBarganingRaw").load("/rAjaxGW/rget.aspx?action=ning-blog-list-CollectiveBarganing", function(response, status, xhr) {
		...
		firstFewItems=jQuery("#hpLatestBlogsCollectiveBarganingRaw div.xg_module_body").filter(":lt(3)");
		...
		jQuery(firstFewItems).appendTo("#hpLatestBlogsCollectiveBarganing");

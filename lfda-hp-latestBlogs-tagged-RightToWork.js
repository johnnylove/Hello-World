...
document.write('<div id="hpLatestBlogsRGGI"></div>');

jQuery(document).ready(function(){
	// Add a "raw zone" to the end of the BODY tag to store the raw data coming from the feed.
	jQuery('body').append('<div id="hpLatestBlogsRGGIRaw" style="display: none;"></div>')
	// Now, get the feed and do some processing once i's done.
	jQuery("#hpLatestBlogsRGGIRaw").load("/rAjaxGW/rget.aspx?action=ning-blog-tagged-RightToWork", function(response, status, xhr) {
		...
		firstFewItems=jQuery("#hpLatestBlogsRGGIRaw div.xg_module_body").filter(":lt(3)");
		...
		jQuery(firstFewItems).appendTo("#hpLatestBlogsRGGI");

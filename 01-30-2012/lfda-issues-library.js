// Adds a feed for the "issues library"

// Output a "visible" zone for the feed (we do this one "in place" so it's placed where the JS executes");
document.write('<div id="lfda_issues_library"></div>');

jQuery(document).ready(function(){
	// Add a "raw zone" to the end of the BODY tag to store the raw data coming from the feed.
	jQuery('body').append('<div id="lfda_issues_library" style="display: none;"></div>')
	// Now, get the feed and do some processing once i's done.
	jQuery("#lfda_issues_libraryRaw").load("/rAjaxGW/rget.aspx?action=lfda-issues-library", function(response, status, xhr) {
		...
		firstFewItems=jQuery("#lfda_issues_libraryRaw div#dnn_ctr14443_HtmlModule_lblContent");
		...
		jQuery(firstFewItems).appendTo("#lfda_issues_library");
